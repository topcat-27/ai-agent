...
The request uses reviewed Person Search fields for current title, current-company industry, and public location. Company-size filtering is performed locally only when a returned current-employment record contains a numeric headcount. The workflow never calls contact enrichment.
